    ...
    static void createTables() {
        try {
            ...
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/", "root", "root");
                ...
                stmt.execute("CREATE DATABASE IF NOT EXISTS db_clinic_0408");
            }
            try (Connection conn = DriverManager.getConnection("jdbc:mysql://localhost:3306/db_clinic_0408", "root", "root");
                 Statement stmt = conn.createStatement()) {
                String sql = new String(Files.readAllBytes(Paths.get("src/com/dentalclinic/test/resources/create.sql")));
